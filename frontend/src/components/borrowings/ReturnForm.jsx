...
export default function ReturnForm({ borrowing, submitting, onSubmit, onCancel }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-5">
      <div>
        <label htmlFor="return_date" className="mb-1 block text-sm font-semibold text-secondary-700">
      ...
      <div>
        <label className="mb-3 block text-sm font-semibold text-secondary-700">Detail Pengembalian per Barang</label>

        <div className="space-y-3">
          {returnItems.map((ri, index) => (
            <div key={ri.borrowing_item_id} className="rounded-xl border border-slate-100 bg-accent-50/50 p-4">
              <div className="mb-3 flex flex-wrap items-center gap-2">
                <span className="rounded-full bg-sky-100 px-3 py-1 text-xs font-bold text-sky-800">
                  {ri.item_code}
                </span>
                <span className="text-sm font-semibold text-secondary-800">{ri.item_name}</span>
